...
const ExpenseComponent = (props) => {
    ...
    const renderExpenseItem = ({ item }) => {
        ...
        return (
            <View style={{ height: 30, justifyContent: 'center', backgroundColor: '#726669', width: screenWidth - 10, margin: 0.5 }}>

                <View style={{ flexDirection: 'row', flex: 10, alignSelf: 'center', justifyContent: 'center', alignItems: 'center' }}>

                    <Text style={{ ...styles.title, flex: 3, padding: 0, fontWeight: 'bold', color: textColor }}> {item.name} </Text>
                    <Text style={{ ...styles.title, flex: 2, padding: 0, fontSize : 12, color: 'white' }}>  ₹ {item.amount} </Text>
                    <Text style={{ ...styles.title, flex: 3, padding: 0, fontSize : 12, color: 'white' }}> {item.category} </Text>
                    <Text style={{ ...styles.title, flex: 2, padding: 0, fontSize : 12, color: 'white' }}> {item.date.substring(0, 6)} </Text>
                    <AppImage
